    ...
    return (
        <>
            {uploadedFiles.map((file, idx) => {
                return (
                    <div>
                        {(file.docTitle !== 'Certificate_of_Incorporation' && file.docTitle !== 'GST_Registration_Certificate') && (
                            <div className="flex gap-3 py-0">
                                <div className="flex-1">
                                    <h4 className="text-white font-light mb-1">{file.docTitle}</h4>
                                </div>
                            </div>
                        )}
                        <div key={idx} className="border border-gray-4f rounded-lg p-4 flex items-center gap-3">
                            ...
                            <div className="flex-1">
                                <h5 className="text-white font-medium text-sm">
                                    {file.url || file.path ? (
                                        <a href={file.url || file.path} target="_blank" rel="noopener noreferrer" className="no-underline hover:text-expona-red">{file.name || file.title}</a>
                                    ) : (
