...
const PdfViewerModal = ({ isOpen, onClose, fileUrl }) => {
  ...
  if (fileName) {
    ...
    if (extMatch) {
      // Cut off anything after the extension
      const extIndex = fileName.indexOf(extMatch[0]) + extMatch[0].length;
  ...
  return (
    <div style={styles.overlay}>
      ...
      <button
        onClick={onClose}
        className="fixed top-5 right-5 bg-gray-32 color-white rounded py-1 px-2 cursor-pointer font-bold"
      >
        ✕
      </button>
        ...
        <div className="text-xs font-light w-72 mx-auto flex justify-between items-center bg-gray-24 px-4 py-2 rounded-full ">
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className="py-1  px-2 bg-gray-32 rounded-lg hover:bg-gray-42"
            >
              &lt;
            </button>
            <input
