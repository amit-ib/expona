...
const RightSideDrawer = ({
  ...
  const [reevaluatedMissedCriteria, setReevaluatedMissedCriteria] =
    useState(null);
  const [reevaluatedMatchedCriteria, setReevaluatedMatchedCriteria] =
    useState(null);
  ...
  const handleEligibilityUpdate = (result) => {
    if (result && result.data && result.data.reevaluated_eligibility) {
      setReevaluatedMissedCriteria(
        result.data.reevaluated_eligibility.MissedCriteria || null
      );
      setReevaluatedMatchedCriteria(
        result.data.reevaluated_eligibility.MatchedCriteria || null
      );
    }
  };
  ...
  return (
    <div
      className={`fixed top-0  overflow-y-auto right-0 h-full w-[465px] bg-gray-24 shadow-lg transform transition-transform duration-300 ease-in-out ${
    ...
    >
      ...
      <div className="relative   w-full flex items-end justify-start bg-gray-800 overflow-hidden1 ">
        <img
        ...
        <div className="relative z-10 px-4 pt-[214px] pb-11  text-center">
          ...
          <p className="text-gray-ae text-sm mt-1">
            You meet {matchedCriteriaCount ?? 0} out of{" "}
            {eligibilityData?.data?.total_criteria ?? 0} criteria. Add turnover
            details and certifications to improve your chances.
          </p>
        </div>
      </div>
      ...
      <TabsComponent
        tabs={[
          {
            label: "Missed Criteria",
            key: "missed",
            count: missedCriteriaCount,
          },
          {
            label: "Matched Criteria",
            key: "matched",
            count: matchedCriteriaCount,
          },
        ]}
        activeTab={activeTab}
      ...
      {/* Tab Content */}
      <div className="flex-1 overflow-y-auto1 px-4 pb-4 mt-5">
        {!eligibilityData ? (
          <SkeletonLoader />
        ) : activeTab === "missed" ? (
          // Missed Criteria
          <div>
            ...
            <div className="space-y-3">
              {(
                reevaluatedMissedCriteria ||
                eligibilityData?.data.report.MissedCriteria ||
                []
              ).map((acc, idx) => (
                <AccordionComponent
                  ...
                  actionType={
                    Array.isArray(acc.actions) && acc.actions.length > 0
                      ? acc.actions[0].action
                      : acc.action
                  }
                  actionDescription={
                    Array.isArray(acc.actions) && acc.actions.length > 0
                      ? acc.actions[0].actionDescription
                      : ""
                  }
                  isOpen={activeTab === "missed" && openAccordion === idx}
                  onToggle={() =>
                    setOpenAccordion(
                      activeTab === "missed" && openAccordion === idx ? -1 : idx
                    )
                  }
          ...
          <div>
            <div className="space-y-3">
              {(
                reevaluatedMatchedCriteria ||
                eligibilityData?.data.report.MatchedCriteria ||
                []
              ).map((acc, idx) => (
                <AccordionComponent
                  key={acc.criteriaName}
                  ...
                  onToggle={() =>
                    setOpenAccordion(
                      activeTab === "matched" && openAccordion === idx
                        ? -1
                        : idx
                    )
                  }
